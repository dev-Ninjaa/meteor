...
export const WalletView: React.FC = () => {
  ...
  // Transactions are user-filtered (usePayments → useMe().id), so direction = f(tx.type)
  const INCOMING_TX_TYPES = ['CLAIM_PAYMENT', 'ESCROW_RELEASE', 'ESCROW_REFUND'];

  const isIncomingTx = (tx: any) => INCOMING_TX_TYPES.includes(tx.type);

  const formatAmount = (tx: any) => {
    const amount = parseFloat(tx.amount);
    const sign = isIncomingTx(tx) ? '+' : '-';
    return `${sign}${amount.toFixed(1)} MON`;
  };

  const formatAmountColor = (tx: any) => {
    return isIncomingTx(tx) ? 'text-emerald-400' : 'text-red-400';
  };
              ...
              transactions.map((tx: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={formatAmountColor(tx)}>
                      {formatAmount(tx)}
                    </div>
                  </div>
                );
